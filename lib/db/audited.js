...
export async function auditedUpdate({ user, model, where, data, entityId }) {
  ...
  console.log("🔍 Raw diff:", JSON.stringify(rawDiff, null, 2));

  const filteredDiff = rawDiff.filter(
    (change) => change.op === "replace" && change.path[0] !== "updatedAt"
  );
  console.log("🧹 Filtered diff:", JSON.stringify(filteredDiff, null, 2));
  ...
  console.log(
    "✅ Final meaningful diff:",
    JSON.stringify(meaningfulDiff, null, 2)
  );
